...
3. Under *Create a New MySQL Database and Database User*:
   - Database name: `energy_talent_cms` (Hostinger prefixes it → `uXXXXXXXXX_energy_talent_cms`)
   - Username: `energy-talent-cms` (→ `uXXXXXXXXX_energy-talent-cms`)
...
```
DATABASE_URL="mysql://uXXXXXXXXX_energy-talent-cms:PASSWORD@srv1234.hstgr.io:3306/uXXXXXXXXX_energy_talent_cms?connection_limit=5"
```
...
For a throwaway local DB instead of Hostinger: `docker run -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=dev -e MYSQL_DATABASE=energy_talent_cms mysql:8` and use `mysql://root:dev@localhost:3306/energy_talent_cms`.
